...
def odometry_callback(msg):
    ...
    pose_lock.release()
    pose = cms.Pose()
    pose.p[0] = msg.pose.pose.position.x
    pose.p[1] = msg.pose.pose.position.y
    pose.p[2] = msg.pose.pose.position.z
    quaternion = np.array([
    ...
    matrix = tft.quaternion_matrix(quaternion)
    pose.x = matrix[0:3,0].tolist()
    pose.y = matrix[0:3,1].tolist()
    pose.z = matrix[0:3,2].tolist()
    pose_pub.publish(pose)
